...
public class SelectionControl2 extends Control {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        for (int i = 1; i <= _lowerPane.getDiagramControl().getMaxElementId(); i++) {
            ...
            if (tempDe != null) {
                if (tempDe.location().x > _ds.location().x && tempDe.location().y > _ds.location().y && (tempDe.location().x + tempDe.size().getWidth()) < (_ds.location().x + _ds.size().getWidth()) && (tempDe.location().y + tempDe.size().getHeight()) < (_ds.location().y + _ds.size().getHeight())) {
        ...
        if (_deArray.size() != 0) {
            ...
            for (int i = 0; i < _deArray.size(); i++) {
                if (((DiagramElement) _deArray.elementAt(i)).getType().getCategory() == DiagramElementType.SHAPE) {
                    // ((DiagramShape)tempDe).showResizePoints(tempDe.getPane(),
                    // (DiagramShape)tempDe);
