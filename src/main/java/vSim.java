...
import diagram.AddConnectorControl;
import diagram.AddShapeControl;
import diagram.CEditor;
import diagram.CompilationFileThread;
import diagram.DefaultShapeControl;
import diagram.Diagram;
import diagram.DiagramConnector;
import diagram.DiagramElement;
import diagram.DiagramElementType;
import diagram.DiagramShape;
import diagram.Frame1;
import diagram.ResizePoint;
import diagram.RunThread;
import diagram.SelectionControl2;
import diagram.UndoCommand;
import engine.FEL;
import engine.SimThread;
import engine.SystemThread;
...
//		        		Class IM2 = Class.forName("InternalModel");
                        // create a custom ClassLoader. You must write the code
                        // for LocalClassLoader.
...
class CBListener extends MouseInputAdapter {
    ...
    private void redispatchMouseEvent(MouseEvent e) {
        ...
        if (_toolbar.contains(point.x, point.y)) {
            ...
            if (component == null) {
                _toolbar.dispatchEvent(new MouseEvent(_toolbar, eventID, e.getWhen(), e.getModifiers(), point.x, point.y, e.getClickCount(), e.isPopupTrigger()));
