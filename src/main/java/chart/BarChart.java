...
public class BarChart extends Presentation implements Serializable
{
	...
	//private BufferedImage bi = new BufferedImage(_immediateSize.width,_immediateSize.height,BufferedImage.TYPE_INT_ARGB);
	...
	public BarChart(Chart chartToShow){
		...
		this.setPreferredSize( _immediateSize );
		this.setBounds(0,0,_immediateSize.width,_immediateSize.height);
		this.setDrawVerticalLine(false);
		this.setDrawBarLabel(false);

		_myMouseAdapter=new BarChartMouseAdapter(this);
		this.addMouseListener(_myMouseAdapter);
		this.addMouseMotionListener(_myMouseAdapter);
	...
	protected void paintComponent(Graphics g) {
		...
		double maxPoint = this.getChart().getMaxSampleValue().getValue();
		...
		this.adjustLineUnit(maxPoint, powCounter);

		_border = this.getInsets();
		int chartHeight;
		chartHeight = this.getHeight() - ((this.getHeight()/10)+ _border.top);
		...
		double scaleFactor;
		scaleFactor = this.calScaleFactor(maxPoint + (double)((Math.pow(10.0,(double)powCounter))*0.5)/5, chartHeight);
		//scaleFactor = this.calScaleFactor(maxPoint + (double)((Math.pow(10.0,(double)powCounter))*0.5)/5, chartHeight);

		if(_isDrawVerticalLine){
			this.drawVerticalLine(g, _space, chartHeight);
		}

		this.drawBars(g, _space, chartHeight, scaleFactor);

		this.drawHorizontalLine(g, maxPoint, powCounter, chartHeight, scaleFactor);

		this.plotLine(g, _space, chartHeight, scaleFactor);

		g.setColor(Color.black);
		this.drawMouseOnBarLabel(g, _mouseOnBarLabel);
		//this.drawLabels(g);
	...
	private void adjustLineUnit(double maxPoint, int powCounter){
		...
		if(maxPoint>=10)
			...
			while(i<length && !match){
				if(!( (String.valueOf(maxPoint)).substring(i,i+1).equals("0") ||
					(String.valueOf(maxPoint).substring(i,i+1)).equals(".")) ){
					// terminate the loop
					length=i;
					match=true;
				}
		...
		if(firstSigDigit > 7){
			_gridLineUnit = (Math.pow(10.0,(double)powCounter));
			_assistantLineUnit = (Math.pow(10.0,(double)powCounter))/5;
		}
		else if(firstSigDigit<=7  && firstSigDigit > 4){
			_gridLineUnit = (Math.pow(10.0,(double)powCounter));
			_assistantLineUnit = (Math.pow(10.0,(double)powCounter))/10;
		}
		else if(firstSigDigit<=4  && firstSigDigit > 2){
			_gridLineUnit = ((Math.pow(10.0,(double)powCounter))*0.5);
			_assistantLineUnit = ((Math.pow(10.0,(double)powCounter))*0.5)/5;
		}
		else{
			_gridLineUnit = ((Math.pow(10.0,(double)powCounter))*0.5);
			_assistantLineUnit = ((Math.pow(10.0,(double)powCounter))*0.5)/10;
		}
	...
	private void drawHorizontalLine(Graphics g, double maxPoint, int powCounter, int chartHeight, double scaleFactor){
		...
		limit = (int)((maxPoint+((long)((Math.pow(10.0,(double)powCounter))*0.5)/5))/ _assistantLineUnit);
		...
		// draw gridLine
		g.setColor(Color.black);
		limit = (int)((maxPoint+((long)((Math.pow(10.0,(double)powCounter))*0.5)/5))/ _gridLineUnit);
	...
	private void drawBars(Graphics g, int space, int chartHeight, double scaleFactor){
		...
		for(i = 0; i < _sampleValues.size(); i++){
			...
			if(_isDrawBarLabel){
				g.drawString(((SampleValue)(_sampleValues.get(_sampleValues.size()-(i+1)))).getLabel(), space * (i+1) - (this.getWidth()/100), chartHeight + (this.getHeight()/20));
			}
			if(_isDrawValueIndicator){
				g.setColor(Color.black);
				g.fillOval(space * (i+1) - ((this.getWidth()/100)/2), (chartHeight-(int)(scaleFactor * ((SampleValue)_sampleValues.get(_sampleValues.size()-i-1)).getValue())) - ((this.getWidth()/100)/2), this.getWidth()/100, this.getWidth()/100);
			}
		}
	}
	...
	*/
	...
	private void drawMouseOnBarLabel(Graphics g, String label){
		...
		g.drawString(label, (this.getWidth()/2)-(strWidth/2), this.getHeight()-strHeight);
	}
