...
//added by matthew
	...
	public DiagramConnector() {
		...
		this.addMouseListener(_moveConnectorControl);
		this.addMouseMotionListener(_moveConnectorControl);
		//Testing!!!!
		this.addKeyListener(_moveConnectorControl);
		_points = new Vector();
		addPoint(0, 0, 0);
		addPoint(0, 0, 0);
...
*/
	...
	public Point findPoint(int x, int y, DiagramElement d) {
		...
		{
			rx = x;
			if (y < y1) // point above the source
				ry = y1;
			else
				ry = y2;
		}
		else if (y1 <= y && y <= y2)  // horizontal line joining d and the current point
		{
			ry = y;
			if (x < x1) // point on the left of d
				rx = x1;
			else
				rx = x2;
		}
	...
	public void addReshapePoint(int x, int y, int index) {  //absolute x, y
		...
		this.getPane().add(reshapePoint, 0);
	...
	public void removeReshapePoint(int index) {
		...
		this.getPane().remove(getReshapePoint(index));
	...
	{
		this.setPoint(x, y, index);
		
		if (index == -1) {
			index = this._points.size()-1;
		}
		
		ReshapePoint reshapePoint = this.getReshapePoint(index);
		reshapePoint.setLocation(x-reshapePoint.getWidth()/2, y-reshapePoint.getHeight()/2);
//		System.err.println("reshapePoint.x" + reshapePoint.getX() + "reshapePoint.y" + reshapePoint.getY());
	...
	public void remove() {
		...
		try {  // 'try' is used to prevent the Source of Connector is Null
			this.getAssociations().getConnectorSource(this).connected(false);
		}
		catch (Exception e) {
		...
		for (int i = 0; i < getReshapePoints().size(); i++) {
			DiagramShape.getDiagram().removeDiagramElement((JComponent) getReshapePoint(i));
		}
	}
