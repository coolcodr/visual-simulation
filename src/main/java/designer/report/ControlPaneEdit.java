...
public class ControlPaneEdit
{
    ...
    public void drawSelected ( Graphics2D g2 )
    {
        ...
        if ( width + height != 0 )
        {
            ...
            if ( status != HEADER_SELECTED
                 && status != FOOTER_SELECTED
                 && status != HEADER_EDITING
                 && status != FOOTER_EDITING
                 && status != HEADER_RESIZING
                 && status != FOOTER_RESIZING
                 && status !=NORMAL )
    ...
    {
        /*
        FontMetrics fm = jTextPaneParagraph.getFontMetrics(jTextPaneParagraph.getFont());
    ...
    public void setDraggedSize ( int x, int y )
    {
        ...
        switch (status)
        {
            case ( ADD_PARAGRAPH ):
                ...
                this.width =  newX;// - newX%gridSize;
                this.height =  newY;// - newY%gridSize;
                //displayTextInfo(String.valueOf(this.width)+" x "+String.valueOf(this.height), this.x, this.y + this.height);
                controlMainEditor.setStatus2Text(" Size: "+String.valueOf(this.width)+" x "+String.valueOf(this.height));
                break;
            case ( PCDIAGRAM_SELECTED ):
                newX = x - dx - marginX + 5;
                newY = y - dy - marginY + 5;
                ...
                //displayTextInfo(String.valueOf(this.x-marginX-5)+", "+String.valueOf(this.y-marginY-5), this.x, this.y-21);
            ...
            case ( PCDIAGRAM_RESIZE ):
                ...
                this.x = selectedDiagram.getX() + marginX - 5;
                this.y = selectedDiagram.getY() + marginY - 5;
                this.width = selectedDiagram.getWidth() + 10;
                this.height = selectedDiagram.getHeight() + 10;
                ...
                controlMainEditor.setStatus2Text(" Size: "+String.valueOf(this.width-10)+" x "+String.valueOf(this.height-10)+" ("+String.valueOf(selectedDiagram.getScale())+"%)");
                break;
            case ( HEADER_RESIZING ):
                newY = y - this.y - 5;
                workPane.getReportPage().setHeaderHeight(newY - newY%gridSize );
                this.height = workPane.getReportPage().getHeaderHeight() - 5;
            ...
            case ( FOOTER_RESIZING ):
                ...
                this.height = workPane.getReportPage().getFooterHeight() -5 ;
            ...
            case ( PCPARAGRAPH_SELECTED ):
                ...
                //displayTextInfo(String.valueOf(this.x-marginX-10)+", "+String.valueOf(this.y-marginY-10), this.x, this.y-21);
            ...
            case ( PCPARAGRAPH_RESIZE ):
                ...
                this.x = selectedParagraph.getX() + marginX;
                this.y = selectedParagraph.getY() + marginY;
                this.width = selectedParagraph.getWidth();
                this.height = selectedParagraph.getHeight();
                //displayTextInfo(String.valueOf(this.width)+" x "+String.valueOf(this.height), this.x, this.y + this.height);
                controlMainEditor.setStatus2Text(" Size: "+String.valueOf(this.width)+" x "+String.valueOf(this.height));
                break;
        }
    ...
    public boolean setDiagramScale ( int i )
    {
        ...
        this.x = diagramX - 5;
        this.y = diagramY - 5;
        this.width = diagramWidth + 10;
        this.height = diagramHeight + 10;
    ...
    public boolean setDiagramScaleHorizontalFit()
    {
        ...
        {
            int scale;// = 0;
            double originalWidth = (double)selectedDiagram.getWidth()/((double)selectedDiagram.getScale()/100);
            scale = (int)(( (double)(imageableWidth - 20) / originalWidth) * 100);
    ...
    public boolean setDiagramScaleVerticalFit()
    {
        ...
        {
            int scale;// = 0;
            double originalHeight = (double)selectedDiagram.getHeight()/((double)selectedDiagram.getScale()/100);
            scale = (int)(( (double)(imageableHeight - workPane.getReportPage().getHeaderHeight() - workPane.getReportPage().getFooterHeight() ) / originalHeight) * 100);
            controlMainEditor.setScaleText(scale);
            return setDiagramScale(scale);
        }
        else if ( selectedParagraph != null )
    ...
    private int selectObject ( int x2, int y2 )
    {
        ...
        PCParagraph[] pcParagraph2 = workPane.getReportPage().getParagraph();
            paragraph: for ( int i = 0 ; i < pcParagraph2.length ; i ++ )
            {
                ...
                if ( x2 > paragraphX
                     && x2 < paragraphX + paragraphWidth
                     && y2 > paragraphY
                     && y2 < paragraphY + paragraphHeight)
                {
                    this.x = paragraphX;
                    this.y = paragraphY;
                    this.width = paragraphWidth;
                    this.height = paragraphHeight;
                    status = PCPARAGRAPH_SELECTED;
        ...
        {
            if ( x2 > this.x + this.width - 3
                 && x2 < this.x + this.width + 6
                 && y2 > this.y + this.height - 3
                 && y2 < this.y + this.height + 6)
            {
                workPane.setCursor(Cursor.getPredefinedCursor(Cursor.SE_RESIZE_CURSOR));
                if ( status == PCDIAGRAM_SELECTED) calStatus = PCDIAGRAM_RESIZE;
                if ( status == PCPARAGRAPH_SELECTED) calStatus = PCPARAGRAPH_RESIZE;
            }
            else if ( x2 > this.x
            ...
            {
                workPane.setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
                if ( status == PCDIAGRAM_SELECTED) calStatus = PCDIAGRAM_SELECTED;
                if ( status == PCPARAGRAPH_SELECTED) calStatus = PCPARAGRAPH_SELECTED;
            }
            else
            {
                workPane.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
        ...
        else
        {
            ...
            if ( x2 > headerX && x2 < headerWidth + headerX
                && y2 > headerY && y2 < headerHeight + headerY
                && workPane.getReportPage().getHeader()!=null)
            {
                this.x = headerX;
                this.y = headerY;
                this.width = headerWidth;
                this.height = headerHeight - 5;
            ...
            if ( x2 > footerX && x2 < footerWidth + footerX
                && y2 > footerY && y2 < footerHeight + footerY
                && workPane.getReportPage().getFooter()!=null)
            {
                this.x = footerX;
                this.y = footerY + 5;
                this.width = footerWidth;
                this.height = footerHeight - 5;
