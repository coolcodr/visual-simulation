...
import designer.deployment.GetDataInput;
import designer.deployment.GetDoubleInput;
import designer.deployment.GetIntegerInput;
import designer.deployment.GetStringInput;
...
public class DiagramComponentSetPanel extends JLayeredPane implements DesignerComponent {
     ...
     * 
     */
    private static final long serialVersionUID = 3135314047415788343L;
    private String name;
    ...
    public void init(String name, Property property) {
        ...
        savedName = name;
        // This is the unique variable name generated, SHOULD BE the same as
        // generte code ( see DiagramElementSource for how this work )
        this.name = name;
        ...
        if (!isPrimitive(property.getType())) {
            ...
            defaultValue = value.substring(value.lastIndexOf(".") + 1); // This
                                                                        // is
                                                                        // wrong
            jButton.setVisible(true);
            this.add(jButton);
            this.add(jComboBox);
            // setComboBoxObject(); //Add later when set the node to this set
            // pane
    ...
    /*
     * public void refreshCardPane () { cardLayout.show(cardPane,
     * ((SimPropertyChoice)jComboBox.getSelectedItem()).getFrameID()); }
     */
    ...
    public void resetCardPaneContents(boolean autoResize) {
        ...
        /*
         * cardPane.removeAll(); Vector designPanes = cardPane.getDesignPanes();
         * for ( int i = 0 ; i < designPanes.size() ; i ++ ) { DesignPane
         * designPane = (DesignPane) designPanes.elementAt(i);
         * ((CardLayout)cardPane.getLayout()).addLayoutComponent(designPane,
         * designPane.getID()); } *. /* //Vector designPanes =
         * DesignerControl.designPanes; Vector designPanes =
         * cardPane.getDesignPanes(); for ( int i = 0 ; i < designPanes.size() ;
         * i ++ ) { DesignPane designPane = (DesignPane)
         * designPanes.elementAt(i); if ( designPane.getParent() != cardPane )
         ...
         * SimPropertyChoice choice = (SimPropertyChoice)jComboBox.getItemAt(i);
         * System
         * .out.println("CHOICE: "+choice+", VALUE: "+choice.getFrameID()); if (
         * ! choice.equals("-1") ) { for ( int j = 0 ; j < designPanes.size() ;
         * j ++ ) { DesignPane designPane = (DesignPane)
         * designPanes.elementAt(j); if (
         * designPane.getID().equalsIgnoreCase(choice.getFrameID() ) ) {
         * System.out.println("ADD: "+designPane.getID());
         * cardPane.add(designPane, designPane.getID() ); } //DesignPane
         * designPane = (DesignPane) DesignerControl.currentDesignPane.
    ...
    public void setCardPane(CardPane cardPane) {
        ...
        // SUPER TEMP
        /*
         * Vector allFrame = DesignerControl.designPanes; for (int i = 1; i <
         * allFrame.size(); i++) { DesignPane designe = (DesignPane)
         * allFrame.elementAt(i); cardPane.add(designe, designe.getID()); }
         */
    }
    ...
    public CComboBox getComboBox() {
        ...
        /*
         * if (!isPrimitive(property.getType())) setComboBoxObject();
         */// refresh those FALSE value in choice
        return jComboBox;
    }
    ...
    /*
     * public void setBounds ( int x, int y, int width, int height ) { int
     * girdValue = DesignerUI.gridValue; x = x - x % girdValue + girdValue / 2;
     * y = y - y % girdValue + girdValue / 2; width = width - width % girdValue
     * + girdValue / 2; height = height = height % girdValue + girdValue / 2;
    ...
    public void autoResize() {
        ...
        if (isPrimitive(property.getType())) {
            ...
            // jTextFieldCover.changeBounds(width/5*2, 0, width/5*3 - grid,
            // height - grid);
            ...
            {
                // jComboBoxCover.changeBounds(width/5*2 , 0, width/5*3-4-28 ,
                // height - grid);

                // jComboBox.setBounds(new Rectangle(width/5*2+5, 5,
                // width/5*3-10-30, height-10));
                // jButton.setVisible(true);
                // jButtonCover.changeBounds(jComboBox.getX()+jComboBox.getWidth()-4,
                // 0, 32, height - grid);
                ...
                // jButton.setBounds(jComboBox.getX()+jComboBox.getWidth()+5, 5,
                // 25, height - 10);
            } else {
                // jComboBoxCover.changeBounds(width/5*2+5, 0, width/5*3-10 -
                // grid, height - grid);
                // jComboBoxCover.changeBounds(width-10-(width/5*3-10 - grid),
                // 0, width/5*3-10 - grid, height - grid);
                jComboBoxCover.changeBounds2(width - 5 - (width / 5 * 3 - grid), 0, width / 5 * 3 - grid, height - grid);
                // jComboBox.setBounds(new Rectangle(width/5*2+5, 5,
                // width/5*3-10, height-10));
            }
    ...
    private void setComboBoxObject() {
        /*
         * PropertiesTableReader pReader = new PropertiesTableReader(); String
         * [] typeAry = pReader.getType(this.property.getType()); String []
         * choiceAry = new String[typeAry.length]; for(int j = 0; j <
         * typeAry.length; j++) { StringTokenizer st = new
         * StringTokenizer(typeAry[j], "."); int count = st.countTokens();
         * while(count != 1) { st.nextToken(); count--; } choiceAry[j] =
         * st.nextToken();
         * 
         * }
         */
...
class InputItemListener implements ItemListener {
    ...
    public InputItemListener(DiagramComponentSetPanel setPane, CardPane cardPane) {
        this.setPane = setPane;
        this.cardPane = cardPane;
        cardLayout = cardPane.getCardLayout();
    }
