...
public class DeployFrame extends DeployComponent {
     ...
     * 
     */
    private static final long serialVersionUID = -1822605476687976627L;
    private String id;
    private String title;
    private String name;
    private JDialog frame;

    private Vector inputObject = new Vector(); // A link only, does not cause
                                               // large size of storage/A link
                                               // only, does not cause large
                                               // size of storage
    private Vector actionObject = new Vector();// A link only, does not cause
                                               // large size of storage
     ...
     * componentPropertise = new InputComponentPropertise ( pane ); this.title =
     * title; this.id = id; this.name = name; }
     */
     ...
     * inputObject = input; actionObject = action; otherObject = other; }
     */
    ...
    public JDialog getFrame(JDialog jFrame) {
        ...
        // int titleHeight =
        // Integer.parseInt((java.awt.Toolkit.getDefaultToolkit().getDesktopProperty("win.frame.captionHeight")).toString());
        // int borderWidth
        // =Integer.parseInt((java.awt.Toolkit.getDefaultToolkit().getDesktopProperty("win.frame.sizingBorderWidth")).toString())
