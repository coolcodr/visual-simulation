...
public class CEditor extends DefaultCellEditor implements CellEditorListener
{
    ...
    public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int r, int c)
    {
        ...
        {
            property = (PropertyCommand) table.getValueAt(table.getSelectedRow(), 0);
            if (property.getEditor()instanceof JTextField)
                editor = new CEditor( (JTextField) property.getEditor());
            else if (property.getEditor()instanceof JComboBox)
                editor = new CEditor( (JComboBox) property.getEditor());
            else if (property.getEditor()instanceof JCheckBox)
                editor = new CEditor( (JCheckBox) property.getEditor());
    ...
    public void editingStopped(ChangeEvent e)
    {
        ...
        //System.out.println(((CEditor)e.getSource()).getCellEditorValue().toString());
        Object test = UIDesigner.sourceTable.getValueAt(UIDesigner.sourceTable.getSelectedRow(),1);
        ((SimPropertyChoice)test).setDisplay(newValue);

        //Refresh combobx cboice
        ((DiagramComponentSetPanel)UIDesigner.sourceList.selectedObject).refreshComboxBoxChoice();
