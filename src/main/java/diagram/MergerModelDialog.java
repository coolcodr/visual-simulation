...
public class MergerModelDialog extends JDialog {

    /**
     * 
     */
    private static final long serialVersionUID = -2239496560570131355L;
    Object _value;
    Property _property;
    ...
    public MergerModelDialog(JFrame owner, Property property, TableTextField txtField) {
        super(owner, "MergerModel Setting", true);
...
//		mainPane.setPreferredSize(new Dimension(400,300));
        ...
        setButton.addActionListener(new MergerModelDialogActionListener(this));
        ...
        JComboBox mergerModelList = new JComboBox(choiceAry);
        mergerModelList.setEditable(false);
        ...
        if (_property.getValue() != null) {
            ...
            System.err.println("mergerModelList.getMaximumRowCount():" + mergerModelList.getMaximumRowCount());
            for (int i = 0; i < mergerModelList.getItemCount(); i++) {
                System.err.println("(String)mergerModelList.getItemAt(i):" + (String) mergerModelList.getItemAt(i));
                if (((String) mergerModelList.getItemAt(i)).equals(classNameOnly)) {
                    mergerModelList.setSelectedIndex(i);
                }
            }
        } else {
            mergerModelList.setSelectedIndex(0);
        }

        mergerModelList.addActionListener(new ActionListener() {
        ...
        JLabel mergerModelLabel = new JLabel("mergerModel Model:");

        JPanel topPane = new JPanel();
        topPane.setLayout(new FlowLayout(FlowLayout.LEFT));
        topPane.add(mergerModelLabel);
        topPane.add(mergerModelList);
        basicPane.add(topPane);

        refreshPane((String) mergerModelList.getSelectedItem());
...
class MergerModelDialogActionListener implements ActionListener {

    private MergerModelDialog _dialog;

    public MergerModelDialogActionListener(MergerModelDialog dialog) {
        _dialog = dialog;
    }
