...
public class PresentationDialog extends JDialog {
     ...
     * 
     */
    private static final long serialVersionUID = 3783794806069403246L;
    Object _value;
    Property _property;
    ...
    public PresentationDialog(JFrame owner, Property property, TableTextField txtField) {
        super(owner, "Presentation Setting", true);
...
//		mainPane.setPreferredSize(new Dimension(400,300));
        ...
        setButton.addActionListener(new PresentationDialogActionListener(this));
        ...
        JComboBox presentationList = new JComboBox(choiceAry);
        presentationList.setEditable(false);
        ...
        if (_property.getValue() != null) {
            ...
            System.err.println("presentationList.getMaximumRowCount():" + presentationList.getMaximumRowCount());
            for (int i = 0; i < presentationList.getItemCount(); i++) {
                System.err.println("(String)presentationList.getItemAt(i):" + (String) presentationList.getItemAt(i));
                if (((String) presentationList.getItemAt(i)).equals(classNameOnly)) {
                    presentationList.setSelectedIndex(i);
                }
            }
        } else {
        ...
        presentationList.addActionListener(new ActionListener() {
        ...
        JLabel presentationLabel = new JLabel("presentation Model:");

        JPanel topPane = new JPanel();
        topPane.setLayout(new FlowLayout(FlowLayout.LEFT));
        topPane.add(presentationLabel);
        topPane.add(presentationList);
        basicPane.add(topPane);

        refreshPane((String) presentationList.getSelectedItem());
...
class PresentationDialogActionListener implements ActionListener {

    private PresentationDialog _dialog;

    public PresentationDialogActionListener(PresentationDialog dialog) {
        _dialog = dialog;
    }
