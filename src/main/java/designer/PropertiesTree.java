...
import table.AbstractTreeTableModel;
...
public class PropertiesTree extends AbstractTreeTableModel implements TreeTableModel {
     ...
     * this((Object)properties); this.properties = properties; } public
     * PropertiesTree ( Object root ) { super(root); }
     */
        ...
        switch (column) {
            ...
            case 1: {
                if (table.getSelectedRow() != -1) {
                    /*
                     * PropertyCommand property =
                     * (PropertyCommand)table.getValueAt(table.getSelectedRow(),
                     * 0); if ( property.getEditor() instanceof JTextField )
                     * table.getColumnModel().getColumn(1).setCellEditor(new
                     * CEditor((JTextField)property.getEditor())); else if (
                     * property.getEditor() instanceof JComboBox )
                     * table.getColumnModel().getColumn(1).setCellEditor(new
                     * CEditor((JComboBox)property.getEditor())); else if (
                     * property.getEditor() instanceof JCheckBox )
                     * table.getColumnModel().getColumn(1).setCellEditor(new
                     * CEditor((JCheckBox)property.getEditor()));
                     */
    ...
    public Object getValueAt(Object node, int column) {
        switch (column) {
            case 0: {
                 ...
                 * ((DefaultMutableTreeNode)node); if ( currentNode.getLevel()
                 * != 1) return null; else return new
                 * Boolean(((SimPropertyChoice
                 * )currentNode.getUserObject()).getDisplay());
                 */
