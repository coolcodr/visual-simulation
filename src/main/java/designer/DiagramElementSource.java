...
import diagram.ComponentInfo;
import diagram.DiagramControl;
import diagram.DiagramElement;
import diagram.DiagramShape;
...
public class DiagramElementSource {
    ...
    public void generateList(String message) {
        ...
        pSetting = new PropertiesSetting();
        ...
        for (int i = 0; i < _elements.size(); i++) {
            ...
            if (c != null) {
                ...
                // Can be use class name also but will so confused in designing.
                // so now commented
                // DefaultMutableTreeNode newNode = new
                // DefaultMutableTreeNode(className.substring(className.lastIndexOf(".")+1));
                ...
                if (className.equalsIgnoreCase("diagram.DiagramChart")) {
                    ...
                    for (int j = 0; j < properties.size(); j++) {
                        ...
                        // Add to list for UI generate //Temp or test or
                        // final...!!
                        ...
                        // Tt is the list of the combobox, such as
                        // "String creator", "Custom creator"...etc
                        String[] typeAry = pReader.getType(type);

                        if (typeAry.length > 0) {
                            // 04 FEB 2003, Now add to the set pane, so that
                            // when click the pane or the node the table tree
                            // will be refreah by get its root.
                            ...
                            for (int k = 0; k < typeAry.length; k++) {
                                ...
                                // For a temp propertise only, it will set much
                                // more later
                                // ///TEMP////
                                Object[] s1 = { "Input Frame", new FrameIDandName("-1", "") };
                                choice.getProperties().add(s1);
                                // ///TEMP////

                                // 04 FEB 2003, Not add to the smae list but in
                                // a new TreeTable
                                // p1Node.add(nestNode);
                                // This will re washed
                                ...
                                // Check if have nest data input ( such as
                                // setName, setUpperLimit)
                                // This checking may be wrong, but it work now,
                                // may be neeed change later
                                // !data[0].getName().equalsIgnoreCase("null")
                                // <=== duno correct or not
                                if (data.length > 0 && !data[0].getName().equalsIgnoreCase("null")) {
                                    for (int l = 0; l < data.length; l++) {
                                        ...
                                        // Nest set pane
                                        String value = null;
                                        // If the default value is same as this
                                        // combobox option, the nested component
                                        // default value
                                        // must set to the value set in the
                                        // diagramming tool
                                        if (setPane1.getDefultValue().toString().equalsIgnoreCase(thisOptionType)) {
                                            // This is the set the defalut
                                            // value, is the type settED is this
                                            value = pSetting.get(((Property) (properties.elementAt(j))).getValue(), data[l].getGetMethod()).toString();
                                        } else {
                                            // This is to set the default to the
                                            // default Class defined is the typ
                                            // setED is not this
                                            try {
                                        ...
                                        // + setPane1Child.getName());
                                        // System.out.println("INTERNAL Set MODE: "
                                        // + setPane1Child.getSetMode());

                                        // Nest Node (User obejct: the Nest set
                                        // pane)
