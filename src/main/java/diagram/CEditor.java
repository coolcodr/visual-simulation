...
public class CEditor extends DefaultCellEditor implements CellEditorListener {
	...
	public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
        ...
        //Here's the code that brings up the dialog.
			...
			_tField = new TableTextField();
			if (_p.getValue() != null) {
				String className = ((String)_p.getValue().getClass().getName());
	...
	public void editingStopped(ChangeEvent e) {
		...
		if(_valueType.equals(TYPE_PRIMITIVE)) {
			try {
				String type = _p.getType();
				//The array of parameter of the constructor, here is the class is String
				...
				// store the old value
				undoCommand = new UndoCommand(_p, oldpValue,o);//new UndoCommand
			...
			boolean found = false;
			String type = _p.getType();
			String fullClassName = null;		// This is the String of full class name e.g. distribution.Uniformation
			...
			if(found) {
				...
				try {
					...
					if ( (_p.getValue() == null) || (_p.getValue().getClass() != Class.forName(fullClassName)) ) {
						o = Class.forName(fullClassName).newInstance();	//This is the object created
					}
					else {
						o = _p.getValue();
					}
